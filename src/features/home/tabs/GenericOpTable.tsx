...
export type SortCol = 'shortOperationId' | 'title' | 'requestPurposeRaw' | 'operationContent' | 'customStatus' | 'executionStatus' | 'techName'

function uniq(arr: string[]) { return [...new Set(arr.filter(Boolean))].sort() }

function sortRows(rows: ProjectRow[], col: SortCol, dir: SortDir, statusField: keyof RuntimeOperationSnapshot): ProjectRow[] {
  return [...rows].sort((a, b) => {
    const va = col === 'shortOperationId' ? a.op.shortOperationId ?? ''
      ...
      : col === 'customStatus' ? a.op[statusField] as string ?? ''
    ...
    const vb = col === 'shortOperationId' ? b.op.shortOperationId ?? ''
      ...
      : col === 'customStatus' ? b.op[statusField] as string ?? ''
...
export function GenericOpTable({
  ops,
  usersData,
  onRowClick,
  statusLabel = 'סטטוס',
  ...
  statusLabel?: string
  statusField?: keyof RuntimeOperationSnapshot
}) {
  ...
  const [fCustomStatus, setFCustomStatus] = useState('')
  ...
  const customStatusOptions = useMemo(() => uniq(rows.map(r => r.op[statusField] as string ?? '')), [rows, statusField])
  ...
  const filtered = useMemo(() => {
    ...
    if (fCustomStatus) out = out.filter(r => r.op[statusField] === fCustomStatus)
    if (fExecStatus) out = out.filter(r => r.op.executionStatus === fExecStatus)
    if (fTech) out = out.filter(r => r.techName === fTech)
    return sortCol ? sortRows(out, sortCol, sortDir, statusField) : out
  }, [rows, fId, fTitle, fPurpose, fContent, fCustomStatus, fExecStatus, fTech, sortCol, sortDir, statusField])
  ...
  const activeFilters = [fId, fTitle, fPurpose, fContent, fCustomStatus, fExecStatus, fTech, sortCol].filter(Boolean).length

  function reset() { setFId(''); setFTitle(''); setFPurpose(''); setFContent(''); setFCustomStatus(''); setFExecStatus(''); setFTech(''); setSortCol(null) }
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, minWidth: 800 }}>
          <thead>
            <tr>
              ...
              <Th label={statusLabel} col="customStatus" sortCol={sortCol} sortDir={sortDir} onSort={handleSort}>
                <FilterSelect value={fCustomStatus} options={customStatusOptions} onChange={setFCustomStatus} />
              </Th>
            ...
            {filtered.map(({ op, techName }) => {
              ...
              return (
                <tr key={op.id} className="op-row" onClick={() => onRowClick(op)} style={{ cursor: 'pointer', background: isDelayed ? 'rgba(255,77,79,0.04)' : isUnassigned ? 'rgba(255,122,0,0.03)' : 'transparent' }}>
                  ...
                  <td style={{ padding: '8px 10px', whiteSpace: 'nowrap' }}><StatusBadge value={op[statusField] as string ?? null} /></td>
