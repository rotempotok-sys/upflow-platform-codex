...
export async function persistRuntimeSnapshotToSupabase(client: any, payload: RuntimeSnapshotPayload) {
  ...
  try {
    ...
    const userRows = payload.users.map((entry) => ({
      ...
      employee_status: entry.employeeStatus,
      can_team: entry.canTeam,
      can_calendar: entry.canCalendar,
      can_clients: entry.canClients,
      can_equipment: entry.canEquipment,
      can_assistant: entry.canAssistant,
      can_ai_ask: entry.canAiAsk,
    ...
    const scheduleEntryRows = payload.scheduleEntries.map((entry) => ({
      ...
      planned_datetime: entry.plannedDateTime ?? null,
      source: 'monday',
      metadata: {
        ...entry.metadata,
